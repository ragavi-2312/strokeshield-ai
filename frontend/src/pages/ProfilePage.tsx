...
export const ProfilePage: React.FC = () => {
  ...
  const expYears = doctor?.experience_years || (isDrRaha ? 8 : 6);
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 mt-0.5">
          AI-generated clinical demo profile for hackathon triage and emergency navigation testing.
        </p>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 pb-6 border-b border-slate-100 text-center sm:text-left">
          {/* Avatar */}
          <div className="w-20 h-20 rounded-3xl bg-gradient-to-tr from-brand-700 via-brand-600 to-teal-500 flex items-center justify-center text-white text-2xl font-black shadow-lg shadow-brand-500/20 shrink-0">
            {doctor?.name ? doctor.name.replace('Dr. ', '').charAt(0) : 'R'}
          </div>

          <div className="space-y-1.5 flex-1">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
              <h2 className="text-xl font-extrabold text-slate-900">{doctor?.name || 'Dr. Raha'}</h2>
              <span className="bg-amber-100 text-amber-900 text-[10px] font-black px-2.5 py-0.5 rounded-full border border-amber-200 uppercase tracking-wider">
                🟡 AI-GENERATED DEMO PROFILE
              </span>
            </div>
            <p className="text-xs text-brand-600 font-semibold flex items-center justify-center sm:justify-start gap-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
          ...
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-1">
            ...
            <div className="flex items-center gap-2">
              <strong className="text-brand-700 font-mono text-sm">{regNumber}</strong>
        ...
        <div className="p-4 bg-amber-50/60 border border-amber-200 rounded-2xl text-xs space-y-1">
          ...
          <p className="text-amber-800 text-[11px] leading-relaxed">
            This profile is an AI-generated fictional identity (<code>data_type: DEMO</code>, <code>data_source: AI_GENERATED</code>) used exclusively for hackathon demonstration. It does not correspond to any real physician or institution.
          </p>
        </div>
        ...
        <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs text-slate-500">
            ...
            <span>Demo Session Active (APP_MODE: DEMO)</span>
          </div>
